...
static void LCD_8BIT_INIT(void)
{
	...
	PinConfig.PIN_number = LCD_D0_PIN;
	PinConfig.MODE = OUTPUT_PP;
	PinConfig.SPEED = SPEED_10;
	MCAL_GPIO_init(LCD_D0_PORT, &PinConfig);

	PinConfig.PIN_number = LCD_D1_PIN;
	PinConfig.MODE = OUTPUT_PP;
	PinConfig.SPEED = SPEED_10;
	MCAL_GPIO_init(LCD_D1_PORT, &PinConfig);

	PinConfig.PIN_number = LCD_D2_PIN;
	PinConfig.MODE = OUTPUT_PP;
	PinConfig.SPEED = SPEED_10;
	MCAL_GPIO_init(LCD_D2_PORT, &PinConfig);

	PinConfig.PIN_number = LCD_D3_PIN;
	PinConfig.MODE = OUTPUT_PP;
	PinConfig.SPEED = SPEED_10;
...
void HAL_LCD_ISBUSY(void)
{
	...
	MCAL_write_PIN(LCD_RW_PORT, LCD_RW_PIN, LOGIC_HIGH);
...
/* ====== LCD is in 8 bit Mode ====== */
    ...
    MCAL_write_PIN(LCD_D0_PORT, LCD_D0_PIN, (command >> 0) & 0x01);
    MCAL_write_PIN(LCD_D1_PORT, LCD_D1_PIN, (command >> 1) & 0x01);
    MCAL_write_PIN(LCD_D2_PORT, LCD_D2_PIN, (command >> 2) & 0x01);
    MCAL_write_PIN(LCD_D3_PORT, LCD_D3_PIN, (command >> 3) & 0x01);
...
void HAL_LCD_WRITE_CHAR(uint8_t character)
{
	...
	//MCAL_write_PIN(LCD_CTRL, RS_SWITCH, LOGIC_HIGH);

	// Set LCD Port using character variable
	MCAL_write_PIN(LCD_D0_PORT, LCD_D0_PIN, (character >> 0) & 0x01);
	MCAL_write_PIN(LCD_D1_PORT, LCD_D1_PIN, (character >> 1) & 0x01);
	MCAL_write_PIN(LCD_D2_PORT, LCD_D2_PIN, (character >> 2) & 0x01);
	MCAL_write_PIN(LCD_D3_PORT, LCD_D3_PIN, (character >> 3) & 0x01);
